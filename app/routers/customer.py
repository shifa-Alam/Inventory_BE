...
def update_customer(
    ...
    db_customer = db.query(Customer).filter(Customer.id == id, Customer.tenant_id == tenant_id).first()
    if not db_customer:
...
def delete_customer(
    ...
    customer = db.query(Customer).filter(Customer.id == id, Customer.tenant_id == tenant_id).first()
    if not customer:
        raise HTTPException(status_code=404, detail="Customer not found")
    db.delete(customer)
    db.commit()
